...
//            @ApiResponse(responseCode = "404", description = "해당하는 게시글은 존재하지 않습니다.",  content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class)))
    ...
    @Parameters(value = {
            ...
            @Parameter(name="memberId", description = "유저 번호", example = "3434")
    })
    ResponseEntity<CommentResponseDTO> saveComment(CommentRequestDTO commentDTO, Long postId, Long memberId);
    ...
    ResponseEntity<CommentResponseDTO> updateComment(CommentRequestDTO commentDTO, Long commentId, Long memberId);
    ...
    ResponseEntity reportComment(Long commentId, Long memberId);
    ...
    ResponseEntity deleteComment(Long commentId, Long memberId);
