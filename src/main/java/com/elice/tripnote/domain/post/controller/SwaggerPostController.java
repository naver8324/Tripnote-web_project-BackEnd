...
public interface SwaggerPostController {
    ...
    @Operation(summary="게시글 조회 - 관리자", description= "게시글을 조회할 때 사용하는 api입니다. 삭제된 게시글도 조회 가능합니다. 게시글 번호가 있으면 특정 게시글을 쓴 멤버의 게시글을 전체 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "게시글 조회에 성공하였습니다.",  content = @Content(mediaType = "application/json")),
    })
    @Parameters(value = {
            @Parameter(name="memberId", description = "게시글 번호", example = "1"),
            @Parameter(name="page", description = "페이지 번호", example = "5"),
            @Parameter(name="size", description = "페이지 크기", example = "30")
    })

    ResponseEntity<Page<PostResponseDTO>> getPostsAll(Long postId, int page, int size);
