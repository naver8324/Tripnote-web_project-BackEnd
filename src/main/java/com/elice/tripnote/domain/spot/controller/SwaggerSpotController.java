...
public interface SwaggerSpotController {
    ...
    @Parameters({
            ...
            @Parameter(name = "landmark", description = "에버랜드, 롯데월드 등 특정 여행지명 검색")
    })
    @ApiResponses(value = {
    ...
    ResponseEntity<?> getSpots(String region, String landmark);
    ...
    @Parameters({
            @Parameter(name = "landmark", description = "특정 여행지명")
    })
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공",
                    content = {@Content(schema = @Schema(implementation = Spot.class))}),
    ...
    ResponseEntity<Spot> increaseLike(String landmark);
    ...
    ResponseEntity<Spot> decreaseLike(String landmark);
}
